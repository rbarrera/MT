...
class Tax_field extends acf_Field
{
	...
	*-------------------------------------------------------------------------------------*/
	...
	function __construct($parent)
	{
		...
		$this->title = __("Categorias chebox",'acf'); // field label (Displayed in edit screens)
	...
	*-------------------------------------------------------------------------------------*/
	...
	function create_field($field)
	{
		...
		if(empty($terms)) {
			...
			if ($field[ 'hidetax' ] == 1){echo '<style id="tax" type="text/css">#'. $field['taxonomy'] .'div {display:none;}</style>';}
			return false;
		}
		...
		<table class="widefat <?php echo $field['class'];?>" data-layout="<?php echo $field['name']; ?>" style="/*padding-bottom:25px;*/background:#fcfcfc;" >
			<tfoot>
				<tr>
					...
					<th colspan="<?php echo $n; ?>" class="<?php echo $field['name']; ?>" style="width:45%;text-align:right;"><span></span></th>
					<th style="width:5%;"><!-- empty --></th>
				</tr>
			</tfoot>
			<tbody>
			...
			foreach( $terms as $term ) {
				...
				<td style="width:<?php echo $w; ?>;border-bottom:1px solid #ededed;">
					<label>
						<input data-categoria="<?php echo $field['taxonomy']; ?>" type="checkbox" class="<?php echo $field['class']; ?> classChangeTax" name="<?php echo $field['name']; ?>" value="<?php echo $term->term_id; ?>" <?php echo $selected; ?>  style="margin-right:5px;" /><?php echo $term->name; ?>
					</label>
				</td>
